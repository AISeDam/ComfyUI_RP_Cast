...
===CHARACTER DETECTION===
Count the number of distinct person trigger keywords in the input:
  Triggers: girl, boy, woman, man, person (anywhere in the text)

If triggers found = 1 (or 0):
  → Output ADDBASE only. No ADDCOL.

If triggers found ≥ 2:
  → Split at the point where the SECOND person trigger appears
  → Text up to (not including) the second trigger = Character 1 → ADDBASE
  → Text from the second trigger onwards = Character 2 → ADDCOL
  → Tags from Character 1 section NEVER appear in ADDCOL
  → Tags from Character 2 section NEVER appear in ADDBASE

===EXAMPLE 1 — single person (no ADDCOL)===
Input: Portrait of a young woman in black ski suit, white goggles on forehead, snow background.
person triggers found: 1 (woman)
Output:
snow background, winter landscape
ADDCOMM
chic, athletic, clean lines
ADDBASE
1woman, black ski suit, white goggles, forehead, snow

===EXAMPLE 2 — two persons (ADDCOL)===
Input: A woman in red coat walks alone on a rainy street. 1girl, blue short hair, red eyes, white plugsuit, pale skin, quiet expression
person triggers found: 2 (woman, girl)
Split at: "1girl"
Output:
rainy street, neon signs, night
ADDCOMM
cinematic, detailed
ADDBASE
1woman, red coat, walking alone
ADDCOL
1girl, blue short hair, red eyes, white plugsuit, pale skin, quiet expression"""
...
class RPConverter:
    ...
    def execute(self, prompt, style_prompt="", lora_directory="", lora_auto_apply=False, ollama_host="http://127.0.0.1:11434", ollama_model="gemma3:12b", debug=False):
        ...
        if use_ollama:
            ...
            # Pre-split: count person trigger keywords to determine character count
            import re as _re
            _triggers = _re.finditer(
                r'\b(girl|boy|woman|man|person)\b', _raw, _re.IGNORECASE)
            _trigger_list = list(_triggers)
            _person_count = len(_trigger_list)

            if _person_count >= 2:
                # Split at the second person trigger
                _split_pos = _trigger_list[1].start()
                _part_a = _raw[:_split_pos].strip().strip(',').strip()
                _part_b = _raw[_split_pos:].strip()
                user_prompt = (
                    f"Convert to RP format. Use ONLY words from the input.\n"
                    f"\n"
                    f"Detected 2 persons. Split the input into two character sections.\n"
                    f"Character 1 section: {_part_a}\n"
                    f"Character 2 section: {_part_b}\n"
                    f"\n"
                    f"Rules:\n"
                    f"- Scene/location tags → before ADDCOMM\n"
                    f"- Character 1 tags → ADDBASE (start with 1girl/1boy/1woman/1man)\n"
                    f"- Character 2 tags → ADDCOL (start with 1girl/1boy/1woman/1man)\n"
                    f"- NEVER mix tags between the two sections\n"
                    f"\n"
                    f"Output:\n"
                )
            else:
                user_prompt = (
                    f"Convert to RP format. Use ONLY words from the input.\n"
                    f"\n"
                    f"Detected 1 person. Output ADDBASE only. No ADDCOL.\n"
                    f"\n"
                    f"Input: {_raw}\n"
                    f"\n"
                    f"Output:\n"
