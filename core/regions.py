...
def make_filters(
    ...
    if usebase:
        ...
        # BASE contributes base_ratio across all COL areas → sum then clamp
        # Actual blending done in denoised_callback; use full=1 here
        base_fil[:] = 1.0
    ...
    for drow in region_rows:
        ...
        for dcell in drow.cols:
            ...
            if not indflip:
                ...
                c_st = int(w * dcell.st)
                c_ed = int(w * dcell.ed)
                # Last cell: correct rounding error
                if row_ed >= 0.999:
                    r_ed = h
                if dcell.ed >= 0.999:
                    c_ed = w
